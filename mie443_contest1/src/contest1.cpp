...
public:
...

    {
        ...
        nav_mode_ = NavMode::EXPLORE;
        aligning_ = false;
        best_left_dist_ = std::numeric_limits<float>::infinity();
        wall_turn_count_ = 0;
        wall_follow_completed_ = false;
...
private:
    ...
    void controlLoop()
    {
        ...
        if (nav_mode_ == NavMode::EXPLORE && !wall_follow_completed_ && haveScan_far_ && bestClearance_far_ < 0.6) {
            nav_mode_ = NavMode::ALIGN_TO_WALL;
            aligning_ = true;
            align_start_yaw_ = yaw_;
            best_align_yaw_ = yaw_;
            best_left_dist_ = std::numeric_limits<float>::infinity();
            RCLCPP_INFO(this->get_logger(), "Reached wall — starting 360 alignment scan");
        }

        if (nav_mode_ == NavMode::ALIGN_TO_WALL && aligning_) {
            // Rotate slowly
            linear_ = 0.0;
            angular_ = 0.4;

            // Track smallest left-wall distance
            if (std::isfinite(min_left_dist_) && min_left_dist_ < best_left_dist_) {
                best_left_dist_ = min_left_dist_;
                best_align_yaw_ = yaw_;
            }

            // Check if full rotation completed
            double angle_rotated = NormAngle(yaw_ - align_start_yaw_);
            if (std::abs(angle_rotated) >= 2.0 * M_PI - deg2rad(5.0)) {
                // Finished 360 scan → rotate back to best yaw
                target_rotation_ = NormAngle(best_align_yaw_ - yaw_);
                start_yaw_ = yaw_;
                aligning_ = false;
                turning_ = true;
                angular_ = (target_rotation_ > 0.0) ? 1.0 : -1.0;
                linear_ = 0.0;
                RCLCPP_INFO(this->get_logger(), "Best wall alignment found — rotating to align");
            }
            return;
        }
        ...
        if (nav_mode_ == NavMode::WALL_FOLLOW) {
            bool left_open  = min_left_dist_ > 0.5;
            bool front_open = min_front_dist_ > 0.5;

            if (left_open) {
                // Turn left
                start_yaw_ = yaw_;
                target_rotation_ = deg2rad(90.0);
                turning_ = true;
                wall_turn_count_++;
                linear_ = 0.0;
                angular_ = 1.0;
            ...
            if (wall_turn_count_ >= 10) {
                nav_mode_ = NavMode::EXPLORE
                wall_follow_completed_ = true;
            }
    ...
    int midpoint_;
    int turn_count_;
    //Additional variables for navigation state and wall following
    NavMode nav_mode_;
    bool aligning_;
    double align_start_yaw_;
    double best_align_yaw_;
    float best_left_dist_;
    int wall_turn_count_;
    bool wall_follow_completed_;
};
